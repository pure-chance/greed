...
impl State {
    pub fn get_best_n(&mut self) -> u32 {
        return self.best_n;
    }
    pub fn get_best_rating(&mut self) -> f64 {
...
impl Greed {
    pub fn get_state(&mut self, player_score: u32, opponent_score: u32, last_turn: bool) -> State {
        if last_turn {
            return self.terminal[opponent_score as usize][player_score as usize];
        } else {
            return self.normal[opponent_score as usize][player_score as usize];
        }
    }
    fn set_state(
        &mut self,
        ...
        if last_turn {
            self.terminal[opponent_score as usize][player_score as usize] = result;
        } else {
            self.normal[opponent_score as usize][player_score as usize] = result;
        }
    }
    pub fn get_terminal_states(
        &mut self,
    ) -> [[State; (MAX_SCORE + 1) as usize]; (MAX_SCORE + 1) as usize] {
        return self.terminal;
    }
    pub fn get_normal_states(
        &mut self,
    ) -> [[State; (MAX_SCORE + 1) as usize]; (MAX_SCORE + 1) as usize] {
        return self.normal;
    }
    ...
    fn calculate_optimal_terminal_action(
        &mut self,
        player_score: u32,
        opponent_score: u32,
    ) -> State {
        ...
        if SIDES * (opponent_score - player_score + 1) <= (MAX_SCORE - player_score) {
            ...
            return State {
                best_n: opponent_score - player_score + 1,
                best_rating: 1.0,
            };
        }
        let max_reasonable_n: u32 =
            ((MAX_SCORE + opponent_score - 2 * player_score) / (SIDES + 1)) + 1;
        let mut optimal_roll: u32 = 0;
        let mut optimal_rating: f64 = 0.0;
        for dice_rolled in 0..=max_reasonable_n {
            let new_rating =
                self.calculate_terminal_rating(player_score, opponent_score, dice_rolled);
    ...
    fn apply_optimal_terminal_actions(&mut self) {
        for player_score in 0..=MAX_SCORE {
            for opponent_score in 0..=MAX_SCORE {
                let result: State =
                    self.calculate_optimal_terminal_action(player_score, opponent_score);
    ...
    */
    ...
    fn calculate_normal_rating(
        ...
        if dice_rolled == 0 {
            return 1.0
                - self
                    .get_state(opponent_score, player_score, true)
                    .get_best_rating();
        }
        let mut rating: f64 = 0.0;
        for sum_dice in dice_rolled..=SIDES * dice_rolled {
            if player_score + sum_dice > MAX_SCORE {
                continue;
            } else {
                let this_probability: f64 = pmf(sum_dice, dice_rolled, SIDES);
                let this_rating: f64 = 1.0
                    - self
                        .get_state(opponent_score, player_score + sum_dice, false)
                        .get_best_rating(); // this should always be already found
    ...
    fn calculate_optimal_normal_action(&mut self, player_score: u32, opponent_score: u32) -> State {
        ...
        for dice_rolled in 0..=max_reasonable_n {
            let new_rating =
                self.calculate_normal_rating(player_score, opponent_score, dice_rolled);
            if new_rating > optimal_rating {
                optimal_roll = dice_rolled;
                optimal_rating = new_rating;
            }
    ...
    fn apply_optimal_normal_actions(&mut self) {
        for order in 0..=2 * MAX_SCORE {
            for place in 0..=min(order, 2 * MAX_SCORE - order) {
                ...
                let temp1: u32 = if order < MAX_SCORE {
                    MAX_SCORE - order
                } else {
                    0
                };
                let temp2: u32 = if order > MAX_SCORE {
                    order - MAX_SCORE
                } else {
                    0
                };
                ...
                let result: State =
                    self.calculate_optimal_normal_action(player_score, opponent_score);
    ...
    pub fn display_terminal_states(&mut self) {
        for i in 0..=MAX_SCORE {
            let f: [f64; (MAX_SCORE + 1) as usize] =
                self.get_terminal_states()[i as usize].map(|mut item| item.get_best_rating());
            println!("{:?}", f)
        }
    }
    pub fn display_normal_states(&mut self) {
        for i in 0..=MAX_SCORE {
            let f: [f64; (MAX_SCORE + 1) as usize] =
                self.get_normal_states()[i as usize].map(|mut item| item.get_best_rating());
            println!("{:?}", f)
        }
    }
    pub fn write_states(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        for player_score in 0..=MAX_SCORE {
            for opponent_score in 0..=MAX_SCORE {
                ...
                terminal_wtr.serialize((
                    player_score,
                    opponent_score,
                    terminal_state.get_best_n(),
                    terminal_state.get_best_rating(),
                ))?;
                normal_wtr.serialize((
                    player_score,
                    opponent_score,
                    normal_state.get_best_n(),
                    normal_state.get_best_rating(),
                ))?;
            }
        }
        terminal_wtr.flush()?;
